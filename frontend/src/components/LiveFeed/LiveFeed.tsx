...
import { BoltIcon, AlertTriangleIcon, ShieldIcon, ClockIcon, CrosshairIcon, SearchIcon, CloseIcon, RefreshIcon } from '../Icons/Icons';
import EmptyState from '../EmptyState/EmptyState';

type FeedTab = 'all' | 'attacks' | 'events' | 'news';

export default function LiveFeed() {
  const { state: threatState, connect: reconnectFeed } = useThreatContext();
        ...
        <div className="live-feed-attacks-section">
          ...
          <div className="live-feed-attacks-grid">
            {activeVectors.length === 0 ? (
              <EmptyState
                compact
                icon={<CrosshairIcon size={32} color="var(--text-muted)" />}
                title={threatState.isConnected ? 'No active attack vectors' : 'Live feed disconnected'}
                message={
                  threatState.isConnected
                    ? "TRINETRA hasn't observed any live attack traffic yet. This updates automatically as threat data streams in."
                    : "Lost connection to the live threat feed. Reconnect to resume streaming attack vectors."
                }
                ctaLabel={threatState.isConnected ? undefined : 'Reconnect'}
                ctaIcon={<RefreshIcon size={12} />}
                onCta={threatState.isConnected ? undefined : reconnectFeed}
              />
        ...
        <div className="live-feed-events-section">
          ...
          <div className="live-feed-events-list">
            {filteredEvents.length === 0 ? (
              feedFilter !== 'all' && allEvents.length > 0 ? (
                <EmptyState
                  compact
                  icon={<ShieldIcon size={32} color="var(--text-muted)" />}
                  title={`No ${feedFilter} to display`}
                  message="Nothing matches this filter right now. Switch back to All to see the full event stream."
                  ctaLabel="Show all events"
                  onCta={() => setFeedFilter('all')}
                />
              ) : (
                <EmptyState
                  compact
                  icon={<ShieldIcon size={32} color="var(--text-muted)" />}
                  title={threatState.isConnected ? 'Waiting for the first event' : 'Live feed disconnected'}
                  message={
                    threatState.isConnected
                      ? 'Threat events and news will appear here automatically as TRINETRA picks them up.'
                      : 'Lost connection to the live threat feed. Reconnect to resume streaming events.'
                  }
                  ctaLabel={threatState.isConnected ? undefined : 'Reconnect'}
                  ctaIcon={<RefreshIcon size={12} />}
                  onCta={threatState.isConnected ? undefined : reconnectFeed}
